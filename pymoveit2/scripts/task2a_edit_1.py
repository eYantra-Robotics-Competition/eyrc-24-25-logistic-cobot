...
class PickAndPlace(Node):
    ...
    def __init__(self):
        ...
        self.tf_buffer = Buffer(cache_time=Duration(sec=300),)
        ...
        self.callback_group_5 = ReentrantCallbackGroup()
    ...
    def go_to_box(self,box_name):
        ...
        if self.turned_left or self.turned_right:
            while ((distance_to_box > 0.01) and (abs(position_box[1] - position_eef[1]) > 0.001)):
            ...
            while ((distance_to_box > 0.01) and ((position_box[0] - position_eef[0]) > 0.001)):
    ...
    def find_transformation(self,source_1,source_2):
        ...
        while transform_stamped is None:
            try:
                transform_stamped = self.tf_buffer.lookup_transform(source_1,source_2,rclpy.time.Time())
            except TransformException as e:
...
def main():
    ...
    node.pick_and_place_control()
    # node.testing()
    # node.alignment_wrist_parallel_to_box()
    # node.find_boxes()
