...
class PickAndPlace(Node):
    ...
    def __init__(self):
        ...
        # buffer = Buffer(cache_time=Duration(seconds=30), max_extrapolation_duration=Duration(seconds=15))
        ...
        self.callback_group_3 = None # ReentrantCallbackGroup()
        self.callback_group_4 = None # ReentrantCallbackGroup()
        self.callback_group_5 = None # ReentrantCallbackGroup()
        self.callback_group_6 = MutuallyExclusiveCallbackGroup()
        ...
        self.create_service(Trigger,"pick_and_place_operation",self.callback_pick_and_place,callback_group=self.callback_group_6)

        self.servo_node = self.create_client(
            srv_type=Trigger,
            srv_name="/servo_node/start_servo",
            callback_group=self.callback_group_5,
        )
    ...
    def callback_pick_and_place(self,request,response):

        self.pick_and_place_control()
        response.success = True  
        response.message = "pick and place done successfully"
        return response    
    ...
    def find_position_of_box(self,box_name):
        ...
        while transform_stamped is None:
            try:
                transform_stamped = self.tf_buffer.lookup_transform("base_link",box_name,rclpy.time.Time(),timeout=rclpy.duration.Duration(seconds=0.0))
    ...
    def get_end_effector_current_pose(self):
        ...
        while transform_stamped is None:
            ...
            try:
                transform_stamped = self.tf_buffer.lookup_transform("base_link","tool0",rclpy.time.Time(),timeout=rclpy.duration.Duration(seconds=0.0))
            except TransformException as e:
    ...
    def pick_box(self,box_name):
        ...
        future = self.gripper_control_magnet_on.call_async(req)
    ...
    def place_box(self,box_name):
        ...
        future = self.gripper_control_magnet_off.call_async(req)
    ...
    def pick_and_place_control(self):
        '''
        Purpose:
        ---
        ...
        print(f"Position of box = {box_with_positions}")
    ...
    def go_to_drop_position(self):
        ...
        # time.sleep(7.0)
        print(self.moveit2.wait_until_executed())
    ...
    def alignment_wrist_parallel_to_box(self):
        ...
        if (wrist_1_link_yaw < math.radians(80)):
            ...
            msg.twist.angular.x = 0.0
            msg.twist.angular.y = 0.0
            msg.twist.angular.z = abs(alignment)*constant
    ...
    def publish_transform(self,source_1,source_2,name):

            try:
                # print("publishing")
                transformation = self.tf_buffer.lookup_transform(source_1,source_2,rclpy.time.Time())
            ...
            #           child_frame_id = 'obj_<marker_id>'          Ex: obj_20, where 20 is aruco marker ID
                ...
                transform_stamped.transform.translation = transformation.transform.translation

                            
                            

                roll,pitch,yaw = tf_transformations.euler_from_quaternion([transformation.transform.rotation.x,transformation.transform.rotation.y,transformation.transform.rotation.z,transformation.transform.rotation.w])
                
                roll = roll + math.radians(-90)
                ...
                print(f"cannot transform from {source_1} to {source_2}")
    ...
    def testing(self):
        ...
        # reference_postion_tool0 = self.find_transformation("tool0","wrist_1_link")
        
        # reference_postion_wrist_1_link = self.find_transformation("wrist_1_link","wrist_2_link")
        # roll = reference_postion_tool0[0] + math.radians(-90)
        # pitch = reference_postion_tool0[1] + math.radians(-90)
        # yaw = reference_postion_tool0[2]

        
        
        # print(math.degrees(reference_postion_wrist_1_link[1][2]))
        # alignment = 1.0
        
        while True:
            # self.publish_transform("base_link","tool0","tool0_ref")
        #     time.sleep(0.2)
            ...
            # reference_postion_wrist_1_link = self.find_transformation("wrist_1_link","tool0")
            # print(math.degrees(reference_postion_wrist_1_link[1][2]))
            # reference_postion_tool0 = self.find_transformation("tool0","wrist_1_link")
            # print(reference_postion_tool0)
            # roll =  reference_postion_tool0[1][0] + math.radians(90)
            # pitch = reference_postion_tool0[1][1] + math.radians(90)
            # yaw =   reference_postion_tool0[1][2]

            # print(f"roll = {roll}, pitch = {pitch}, yaw = {yaw}")


            # reference_postion_tool0 = self.find_transformation("tool0","wrist_1_link")
            # print(math.degrees(reference_postion_tool0[1][2]))
        ...
        # home_position = [0.0, 0.0, 0.0, -3.1501924424701175, -1.5799040814321978, 0.0]
        # self.moveit2.move_to_configuration(home_position)
        # self.moveit2.wait_until_executed()
